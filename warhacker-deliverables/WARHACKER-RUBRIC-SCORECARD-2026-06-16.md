...
| Portability | 25% | STRONG — digest-pinned bundle **published + keyless-signed + SBOMed + SLSA-attested**, installs on any UDS Core; small-box overrides. Full-fleet breadth = yupana-blocked gap |
| Death Proof | 25% | STRONG — **prove-coboot GREEN** + **prove-organs GREEN for a11oy & killinchu** (individual deployability) under cosign-verify + verify-attestation + airgap gates; receipts/Prometheus/Grafana + guard suite. prove-bundle-install 4/5 green, yupana blocked |
...
1. **yupana organ image is broken (the one real blocker).** prove-bundle-install #30
   (run 27592186983) proved 4/5 organs install->health-200 from the published, signed,
   attested bundle (a11oy, killinchu, amaru, sentra — ALL gates PASS). **yupana** fails:
   `python: can't open file '/home/user/app/app.py': [Errno 13] Permission denied`
   -> `szl-yupana` Deployment never reaches Available. The fault is baked into the
   prebuilt image `ghcr.io/szl-holdings/yupana:uds-v0.2.0@sha256:1984a15f...` (no in-repo
   Dockerfile). **Close:** rebuild the yupana image so `app.py` is world-readable /
   ...
   gate-weakened (yupana stays in the matrix and honestly red).*
2. **prove-organs nightly — FIXED via THREE sequential parity fixes, now GREEN.**
   ...
   the shipped bundle); yupana stays red by design (gap 1) — the harness honestly
...
3. **Standalone / full-fleet breadth.** The consolidated `szl-uds-bundle:uds-v0.3.0`
   ...
   fleet prove-install is blocked only by yupana (gap 1).
...
every gate green. The single honest blocker to the full 5-organ claim is the yupana
