...
#   * an UNKNOWN namespace present must page EXACTLY once (the OK->ALERT edge);
#   * while still present it must DE-DUPE (page zero more times), never re-page;
...
# `szl-ns-scratch list-unlabeled`, and a capturing notifier) and actually runs
# box-scripts/sbin/szl-ns-scratch-watch against fixture scenarios, then asserts
# the observed behaviour. The static checks (exists/parses, shells out, wired,
# documented) mirror the sibling ns-scratch-stale-watch-guard.
#
...
# `szl-ns-scratch list-unlabeled`, and echo its path. The real watch script is
...
#   SCEN       : list-unlabeled fixture — present | none | nothing
_mk_sandbox() {
  local d; d="$(mktemp -d)"
...
EOS
  ...
  cat >"$d/scratch" <<'EOS'
#!/usr/bin/env bash
# Fake `szl-ns-scratch`. Only list-unlabeled is exercised. The real tool prints
# one bare namespace name per line on stdout (UNKNOWN/unlabeled set).
[ "$1" = "list-unlabeled" ] || exit 0
case "${SCEN:-none}" in
  present)
    printf '%s\n' "szl-foo" "szl-bar" ;;
  none) : ;;
...
# _run TARGET SANDBOX SCEN [K3D_RC] [READYZ_RC] — run the real watch script
...
#   A  unknown present, fresh state   -> ALERT, page EXACTLY once
...
chk3() {
  ...
  # A — unknown present, fresh state.
  : >"$cap"
  _run "$F" "$d" present
  [ "$RUN_RC" -eq 0 ] || { err "$F" "REGRESSION — run errored (rc=$RUN_RC) when an untracked scratch namespace was present."; rm -rf "$d"; return 1; }
  n="$(_pages "$cap")"
  [ "$n" -eq 1 ] || { err "$F" "REGRESSION — an untracked scratch namespace must page EXACTLY once on the OK->ALERT edge (got $n)."; rm -rf "$d"; return 1; }
  grep -Fq 'WITHOUT the szl.io/ephemeral label' "$cap" || { err "$F" "REGRESSION — the ALERT page lost its 'WITHOUT the szl.io/ephemeral label' body."; rm -rf "$d"; return 1; }
  grep -q '"overall":"ALERT"' "$d/state/status.json" 2>/dev/null || { err "$F" "REGRESSION — status.json is not ALERT while an untracked namespace is present."; rm -rf "$d"; return 1; }
  ...
  # B — still present (prev ALERT): must de-dupe.
  : >"$cap"
  _run "$F" "$d" present
  n="$(_pages "$cap")"
  [ "$n" -eq 0 ] || { err "$F" "REGRESSION — de-dupe broken: re-paged while still present (got $n, want 0). This is the alert-storm failure."; rm -rf "$d"; return 1; }

  # C — none present (prev ALERT): RECOVERED once.
  : >"$cap"
  _run "$F" "$d" none
  n="$(_pages "$cap")"
  [ "$n" -eq 1 ] || { err "$F" "REGRESSION — recovery (all untracked cleaned up) must page EXACTLY once (got $n)."; rm -rf "$d"; return 1; }
  ...
  rm -rf "$d"
  echo "OK: edge lifecycle — unknown->ALERT(1), still-present de-dupe(0), recovered(1), steady-OK(0)"
}
...
# the owner first; this alarm only surfaces the UNKNOWN set. Asserted both
# statically (no `kubectl delete` invocation in the source) and behaviourally (no
# destructive call reaches the stub cluster during an alert run).
chk5() {
  local root="${1:-.}"
  local F="$root/$WATCH_REL"
  test -f "$F" || { err "$F" "missing — required for the ns-scratch-watch guard"; return 1; }

  # Match an actual command invocation at line-start, NOT operator-guidance text
  # inside the alert body (which may legitimately quote a 'kubectl delete' hint).
  if grep -Eq '^[[:space:]]*kubectl[[:space:]]+delete' "$F"; then
    err "$F" "REGRESSION — szl-ns-scratch-watch now runs 'kubectl delete'. This guard must NEVER auto-delete a namespace; cleanup confirms with the owner."
    return 1
  fi
