#!/usr/bin/env bash
# Copyright 2026 SZL Holdings
# SPDX-License-Identifier: Apache-2.0
#
# receipt-throttle-watch-selftest.sh — prove the receipt-throttle-watch box
# alarm (box-scripts/sbin/receipt-throttle-watch) ACTUALLY FIRES when the
# receipts-server is sustainedly shedding POST /receipt traffic (ingest rate
# limiter throttling a runaway source), and behaves correctly on every edge, by
# DRIVING the real script with a stubbed kubectl/k3d/metrics source. No cluster,
# no network — pure local fixtures.
#
# WHY a behavioural self-test (vs the text/lint guard pattern some sibling box
# alarms use): the value of receipt-throttle-watch is its RATE/EDGE MATH — the
# shed-requests/min computation off the szl_receipts_throttled_total counter,
# the counter-reset=>rate-0 guard (pod restart), the baseline-only first run,
# the MIN_INTERVAL skip, the cluster-absent no-op, and the
# EXACTLY-ONE-notify-per-edge de-dup. A grep guard can confirm a line still
# exists; only running the script proves the math still produces the right
# ALERT/OK/UNKNOWN verdict — and that the alert NAMES the offending subject.
# Mirrors scripts/receipt-flood-watch-selftest.sh.
#
# It feeds the script:
#   * a stub `kubectl` that answers /readyz, the deploy presence check, the pod
#     listing, the `exec ... /metrics` read (emitting a controllable
#     szl_receipts_throttled_total counter via $STUB_THROTTLED), and the
#     `exec ... python3 ...os.walk...` subject-read (emitting $STUB_SUBJECT);
#   * a stub `k3d` that fails (to simulate an absent cluster);
#   * a stub notifier that counts invocations AND records each message so we can
#     assert the offending subject is named in the page.
# and seeds the per-cluster SAMPLE_FILE to control the prior sample/timestamp so
# the rate is deterministic without waiting MIN_INTERVAL_SECS of wall-clock.
#
# Scenarios asserted:
#   1. baseline-only on first run (no prior sample) -> OK, sample recorded, no page
#   2. ALERT when shed/min >= THROTTLE_PER_MIN, and the page NAMES the subject
#   3. OK when shed/min <  THROTTLE_PER_MIN
#   4. rate 0 on a counter reset (count drops, pod restart) -> OK (no false alarm)
#   5. no-op when the cluster is absent (exit 0, UNKNOWN, no sample, no page)
#   6. EXACTLY ONE notify on each edge (OK->ALERT once, dedup while ALERT,
#      RECOVERED once, silent while OK)
set -uo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="${REPO_ROOT:-$HERE/..}"
SCRIPT="$REPO_ROOT/box-scripts/sbin/receipt-throttle-watch"

PASS=0; FAIL=0
ok()   { printf '  ok   %s\n' "$1"; PASS=$((PASS+1)); }
bad()  { printf '  FAIL %s\n' "$1"; FAIL=$((FAIL+1)); }

[ -f "$SCRIPT" ] || { echo "::error::missing $SCRIPT"; exit 1; }
bash -n "$SCRIPT" || { echo "::error::$SCRIPT does not parse"; exit 1; }

# ---- Stub toolchain (kubectl / k3d / notifier) ------------------------------
STUBBIN="$(mktemp -d)"
trap 'rm -rf "$STUBBIN"' EXIT

cat >"$STUBBIN/kubectl" <<'KEOF'
#!/usr/bin/env bash
# Minimal kubectl stub. Order matters: match the most specific first.
#   * the SUBJECT read exec carries the marker 'os.walk' in its python -c body
#     -> emit $STUB_SUBJECT (the dominant accepted-receipt subject line).
#   * the METRICS read exec carries 'urlopen' -> emit the throttled counter.
all="$*"
case "$all" in
  *"--raw=/readyz"*)
    exit 0 ;;                                   # cluster reachable
  *" exec "*"os.walk"*)
    printf '%s\n' "${STUB_SUBJECT:-}"           # subject identification read
    exit 0 ;;
  *" exec "*"urlopen"*|*" exec "*"/metrics"*)
    printf '# HELP szl_receipts_throttled_total POST /receipt requests shed\n'
    printf '# TYPE szl_receipts_throttled_total counter\n'
    printf 'szl_receipts_throttled_total %s\n' "${STUB_THROTTLED:-0}"
    exit 0 ;;
  *" logs "*)
    printf '%s\n' "${STUB_PEPRLOG:-}"           # pepr Throttling log lines
    exit 0 ;;
  *"get pods"*)
    printf '%s\n' "${STUB_PODS:-}"
    exit 0 ;;
  *"get deploy szl-receipts-server"*)
    exit 0 ;;                                   # receipts module present
  *)
    exit 0 ;;
esac
KEOF
chmod +x "$STUBBIN/kubectl"

# k3d stub: always FAILS, so an unset KUBECONFIG_FILE simulates an absent cluster.
cat >"$STUBBIN/k3d" <<'K3EOF'
#!/usr/bin/env bash
exit 1
K3EOF
chmod +x "$STUBBIN/k3d"

# notifier stub: append the message to $NOTIFY_MSGS, one record per invocation.
cat >"$STUBBIN/notify-stub" <<'NEOF'
#!/usr/bin/env bash
msg="$(cat)"
printf '%s\n---\n' "$msg" >> "$NOTIFY_MSGS"
NEOF
chmod +x "$STUBBIN/notify-stub"

READY_POD='{"items":[{"metadata":{"name":"szl-receipts-server-test"},"status":{"containerStatuses":[{"name":"receipts-server","ready":true}]}}]}'

# run_watch: drives the real script against a state dir with our stubs on PATH.
run_watch() {
  PATH="$STUBBIN:$PATH" \
  CLUSTER=test \
  STATE_DIR="$STATE" \
  LOG_DIR="$STATE/log" \
  NOTIFY_CMD="$STUBBIN/notify-stub" \
  ALERT_PREFIX="" \
  bash "$SCRIPT"
}

notify_count() { [ -r "$NOTIFY_MSGS" ] && grep -c '^---$' "$NOTIFY_MSGS" || echo 0; }
status_file()  { echo "$STATE/test.status.json"; }
sample_file()  { echo "$STATE/test.sample"; }
last_file()    { echo "$STATE/test.last_status"; }
seed_sample()  { printf '%s %s\n' "$1" "$2" > "$(sample_file)"; }   # count ts

echo "== receipt-throttle-watch behavioural self-test =="

# ---------------------------------------------------------------------------
# 1. Baseline-only on first run (no prior sample)
# ---------------------------------------------------------------------------
STATE="$(mktemp -d)"; NOTIFY_MSGS="$STATE/notify.msgs"; export NOTIFY_MSGS
KUBECONFIG_FILE="$STATE/kubeconfig"; export KUBECONFIG_FILE; : > "$KUBECONFIG_FILE"
export STUB_PODS="$READY_POD" STUB_THROTTLED=500
rc=0; run_watch >/dev/null 2>&1 || rc=$?
[ "$rc" -eq 0 ] && ok "baseline: exit 0" || bad "baseline: expected exit 0, got $rc"
grep -q '"overall":"OK"' "$(status_file)" && ok "baseline: status OK" || bad "baseline: status not OK"
grep -q 'baselined' "$(status_file)" && ok "baseline: reason says baselined" || bad "baseline: reason missing 'baselined'"
[ -r "$(sample_file)" ] && grep -q '^500 ' "$(sample_file)" && ok "baseline: sample recorded" || bad "baseline: sample not recorded"
[ "$(notify_count)" -eq 0 ] && ok "baseline: no page" || bad "baseline: paged on first run"
rm -rf "$STATE"

# ---------------------------------------------------------------------------
# 2. ALERT when shed/min >= THROTTLE_PER_MIN, page NAMES the subject
#    seed prev=500 @ now-120s, cur=900 -> 400 over ~120s = ~200/min >= 10
# ---------------------------------------------------------------------------
STATE="$(mktemp -d)"; NOTIFY_MSGS="$STATE/notify.msgs"; export NOTIFY_MSGS
export KUBECONFIG_FILE="$STATE/kubeconfig"; : > "$KUBECONFIG_FILE"
export STUB_PODS="$READY_POD" STUB_THROTTLED=900
export STUB_SUBJECT="keycloak-system/StatefulSet/keycloak(x18)"
seed_sample 500 "$(( $(date -u +%s) - 120 ))"
rc=0; run_watch >/dev/null 2>&1 || rc=$?
[ "$rc" -eq 0 ] && ok "alert: exit 0" || bad "alert: expected exit 0, got $rc"
grep -q '"overall":"ALERT"' "$(status_file)" && ok "alert: status ALERT" || bad "alert: status not ALERT"
grep -q '>= 10/min' "$(status_file)" && ok "alert: reason cites threshold" || bad "alert: reason missing threshold"
[ "$(cat "$(last_file)")" = "ALERT" ] && ok "alert: edge state=ALERT" || bad "alert: edge state not ALERT"
[ "$(notify_count)" -eq 1 ] && ok "alert: paged exactly once" || bad "alert: notify count $(notify_count) != 1"
grep -q 'keycloak-system/StatefulSet/keycloak' "$NOTIFY_MSGS" && ok "alert: page NAMES the offending subject" || bad "alert: subject not named in page"
rm -rf "$STATE"

# ---------------------------------------------------------------------------
# 3. OK when shed/min < THROTTLE_PER_MIN
#    seed prev=500 @ now-120s, cur=510 -> 10 over ~120s = ~5/min < 10
# ---------------------------------------------------------------------------
STATE="$(mktemp -d)"; NOTIFY_MSGS="$STATE/notify.msgs"; export NOTIFY_MSGS
export KUBECONFIG_FILE="$STATE/kubeconfig"; : > "$KUBECONFIG_FILE"
export STUB_PODS="$READY_POD" STUB_THROTTLED=510
seed_sample 500 "$(( $(date -u +%s) - 120 ))"
rc=0; run_watch >/dev/null 2>&1 || rc=$?
[ "$rc" -eq 0 ] && ok "below: exit 0" || bad "below: expected exit 0, got $rc"
grep -q '"overall":"OK"' "$(status_file)" && ok "below: status OK" || bad "below: status not OK"
grep -q '< 10/min' "$(status_file)" && ok "below: reason under threshold" || bad "below: reason not under threshold"
[ "$(cat "$(last_file)")" = "OK" ] && ok "below: edge state=OK" || bad "below: edge state not OK"
[ "$(notify_count)" -eq 0 ] && ok "below: no page" || bad "below: paged under threshold"
rm -rf "$STATE"

# ---------------------------------------------------------------------------
# 4. rate 0 on a counter reset (count drops, pod restart) -> OK, no false alarm
#    seed prev=500 @ now-120s, cur=5 (reset) -> delta negative -> rate 0.00
# ---------------------------------------------------------------------------
STATE="$(mktemp -d)"; NOTIFY_MSGS="$STATE/notify.msgs"; export NOTIFY_MSGS
export KUBECONFIG_FILE="$STATE/kubeconfig"; : > "$KUBECONFIG_FILE"
export STUB_PODS="$READY_POD" STUB_THROTTLED=5
seed_sample 500 "$(( $(date -u +%s) - 120 ))"
rc=0; run_watch >/dev/null 2>&1 || rc=$?
[ "$rc" -eq 0 ] && ok "reset: exit 0" || bad "reset: expected exit 0, got $rc"
grep -q '"overall":"OK"' "$(status_file)" && ok "reset: status OK" || bad "reset: status not OK"
grep -q 'throttling 0.00 ' "$(status_file)" && ok "reset: rate reported as 0.00 (zeroed)" || bad "reset: rate not zeroed on reset"
[ "$(notify_count)" -eq 0 ] && ok "reset: no false page" || bad "reset: false alarm on reset"
rm -rf "$STATE"

# ---------------------------------------------------------------------------
# 5. no-op when the cluster is absent (k3d stub fails, no KUBECONFIG_FILE)
# ---------------------------------------------------------------------------
STATE="$(mktemp -d)"; NOTIFY_MSGS="$STATE/notify.msgs"; export NOTIFY_MSGS
export KUBECONFIG_FILE=""        # force the k3d resolve path -> stub fails
export STUB_PODS="$READY_POD" STUB_THROTTLED=999999
rc=0; run_watch >/dev/null 2>&1 || rc=$?
[ "$rc" -eq 0 ] && ok "absent: exit 0 (no-op)" || bad "absent: expected exit 0, got $rc"
grep -q '"overall":"UNKNOWN"' "$(status_file)" && ok "absent: status UNKNOWN" || bad "absent: status not UNKNOWN"
grep -q 'cluster absent' "$(status_file)" && ok "absent: reason 'cluster absent'" || bad "absent: reason wrong"
[ ! -e "$(sample_file)" ] && ok "absent: did not disturb the sample baseline" || bad "absent: wrote a sample on an absent cluster"
[ "$(notify_count)" -eq 0 ] && ok "absent: no page" || bad "absent: paged on an absent cluster"
rm -rf "$STATE"

# ---------------------------------------------------------------------------
# 6. EXACTLY ONE notify on each edge (persistent state + counter across runs)
#    OK->ALERT (page) -> still ALERT (dedup) -> RECOVERED (page) -> still OK (silent)
# ---------------------------------------------------------------------------
STATE="$(mktemp -d)"; NOTIFY_MSGS="$STATE/notify.msgs"; export NOTIFY_MSGS
export KUBECONFIG_FILE="$STATE/kubeconfig"; : > "$KUBECONFIG_FILE"
export STUB_PODS="$READY_POD" STUB_SUBJECT="ns/Deployment/runaway(x40)"

# run1: OK->ALERT edge
export STUB_THROTTLED=900; seed_sample 500 "$(( $(date -u +%s) - 120 ))"
run_watch >/dev/null 2>&1
c1="$(notify_count)"
# run2: still ALERT -> dedup, no new page
export STUB_THROTTLED=900; seed_sample 500 "$(( $(date -u +%s) - 120 ))"
run_watch >/dev/null 2>&1
c2="$(notify_count)"
# run3: ALERT->OK edge (RECOVERED) -> page
export STUB_THROTTLED=510; seed_sample 500 "$(( $(date -u +%s) - 120 ))"
run_watch >/dev/null 2>&1
c3="$(notify_count)"
# run4: still OK -> silent
export STUB_THROTTLED=510; seed_sample 500 "$(( $(date -u +%s) - 120 ))"
run_watch >/dev/null 2>&1
c4="$(notify_count)"

[ "$c1" -eq 1 ] && ok "edge: OK->ALERT paged once"            || bad "edge: OK->ALERT count $c1 != 1"
[ "$c2" -eq 1 ] && ok "edge: held ALERT deduped (no re-page)" || bad "edge: ALERT re-paged ($c2 != 1)"
[ "$c3" -eq 2 ] && ok "edge: ALERT->OK (RECOVERED) paged once" || bad "edge: RECOVERED count $c3 != 2"
[ "$c4" -eq 2 ] && ok "edge: held OK stays silent"            || bad "edge: OK paged ($c4 != 2)"
rm -rf "$STATE"

echo "== summary: $PASS passed, $FAIL failed =="
[ "$FAIL" -eq 0 ] || exit 1
