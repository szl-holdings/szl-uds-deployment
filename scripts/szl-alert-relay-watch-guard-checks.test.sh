...
#
# Self-test (negative fixtures) for szl-alert-relay-watch-guard-checks.sh.
...
CHECKS="$HERE/szl-alert-relay-watch-guard-checks.sh"

# shellcheck source=/dev/null
source "$CHECKS"

SRC_FILES=(
  "box-scripts/sbin/szl-alert-relay-watch"
...
W="box-scripts/sbin/szl-alert-relay-watch"

# chk1 — script missing
d="$(new_fixture)"; rm -f "$d/$W"
expect_fail chk1 "$d" "chk1 fails when szl-alert-relay-watch is missing"
rm -rf "$d"
...
# chk2 — drop the /relay/health probe
d="$(new_fixture)"; sed -i 's#/relay/health#/DISABLED#g; s/"\$http_code" = "200"/"\$http_code" = "DISABLED"/g' "$d/$W"
expect_fail chk2 "$d" "chk2 fails when the /relay/health probe is removed"
rm -rf "$d"

# chk3 — drop the is-active probe
d="$(new_fixture)"; sed -i 's/is-active "\$RELAY_UNIT"/is-DISABLED "\$RELAY_UNIT"/g; s/"\$unit_state" = "active"/"\$unit_state" = "DISABLED"/g' "$d/$W"
expect_fail chk3 "$d" "chk3 fails when the is-active unit probe is removed"
rm -rf "$d"
...
d="$(new_fixture)"; sed -i 's/systemctl enable --now szl-alert-relay-watch.timer/# DISABLED/' "$d/box-scripts/install.sh"
expect_fail chk6 "$d" "chk6 fails when install.sh stops enabling the timer"
rm -rf "$d"

# chk7 — remove the README documentation
d="$(new_fixture)"; sed -i 's/Watch the relay/DISABLED the relay/g' "$d/box-scripts/szl-alert-relay.README.md"
expect_fail chk7 "$d" "chk7 fails when the README drops the watchdog's description"
rm -rf "$d"
