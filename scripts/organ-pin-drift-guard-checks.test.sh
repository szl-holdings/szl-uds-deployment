...
#
# organ-pin-drift-guard-checks.test.sh — negative-fixture self-test for the ORGAN
# pin-drift PAGER guard (scripts/organ-pin-drift-guard-checks.sh).
#
# WHY THIS EXISTS
...
# copy of organ-pin-drift-guard.yml and asserts the check FAILS, plus asserts the
# pristine repo PASSES. Mirrors scripts/pin-drift-guard-checks.test.sh.
#
# Usage: bash scripts/organ-pin-drift-guard-checks.test.sh
...
CHECKS="$HERE/organ-pin-drift-guard-checks.sh"
...
d="$(new_fixture)"; sed -i '/echo "drift=1"/d; /echo "drift=0"/d' "$d/$WF"
expect_fail chk4 "$d" "chk4 fails when the drift output export is removed"

d="$(new_fixture)"; sed -i '/echo "stale<<EOF"/d' "$d/$WF"
expect_fail chk4 "$d" "chk4 fails when the stale output export is removed"
...
echo ""
echo "==================================================================="
echo "organ-pin-drift-guard self-test: $PASS passed, $FAIL failed"
echo "==================================================================="
[ "$FAIL" -eq 0 ]
